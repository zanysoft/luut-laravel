...
<html lang="en">
<head>
    <?php /*echo '<?php $baseUrl = config("app.url") ?>'; */ ?>
    ...
    <script>
        var endpint = "{{ $tryItOut['base_url'] ?? config('app.url') }}";
    </script>
                ...
                return preflightPromise.then(() => makeAPICall(method, path, body, query, headers, endpointId))
                    .then(([responseStatus, statusText, responseContent, responseHeaders]) => {
                        ...
                        try {
                            ...
                            if (jsonParsed !== null) {
                                if ((path == 'api/auth/login' || path == 'api/login') && jsonParsed.token) {
                                    setCookie('api-token', jsonParsed.token, 1);
                                    document.querySelectorAll('.auth-value').forEach(el => {
                                        el.value = jsonParsed.token;
    ...
    <script>
        ...
        addEventListener('DOMContentLoaded', () => {
            ...
            var apiToken = getCookie('api-token');
            ...
            var eurl = new URL(endpint);
            if (curl.host != eurl.host) {
                var _curl = curl.protocol + "//" + curl.host;
                var _eurl = eurl.protocol + "//" + eurl.host;

                console.log(curl, eurl,_eurl, _curl)
                tryItOutBaseUrl = tryItOutBaseUrl.replaceAll(_eurl, _curl);
                document.querySelectorAll('.example-request-javascript,.example-request-php,.endpoint-base,.introduction').forEach(el => {
                    el.innerHTML = el.innerHTML.replaceAll(_eurl, _curl);
                });
                //document.body.innerHTML = document.body.innerHTML.replaceAll(endpointHost, currentHost);
            }
        });
        ...
        <div class="sl-mb-10">
            <div class="sl-mb-4">
                <h1 class="sl-text-5xl sl-leading-tight sl-font-prose sl-font-semibold sl-text-heading">
                    {!! $metadata['title'] !!}
                    <span class="is-logged-in"
                          style="display: none; font-size: 15px; vertical-align: middle;color: green;">Logged In</span>
                </h1>
            ...
            <div class="sl-prose sl-markdown-viewer introduction sl-my-4">
                {!! $intro !!}
